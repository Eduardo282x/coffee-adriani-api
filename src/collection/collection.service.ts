...
export class CollectionService {
  ...
  async getClientReminder() {
    try {
      const invoicesExpired: ResponseInvoice =
        (await this.invoiceService.getInvoicesExpired()) as ResponseInvoice;
  ...
  async markMessageClients(messageId: number) {
    try {
      const findMessageSelected = await this.prismaService.message.findFirst({
        where: { id: messageId },
      });
  ...
  async addClientToCollection() {
    try {
      const invoicesExpired =
        (await this.invoiceService.getInvoicesExpired()) as unknown as ResponseInvoice;
  ...
  async sendMessages() {
    try {
      ...
      const removeDuplicates = [
        ...new Map(reminder.map((item) => [item.client.phone, item])).values(),
      ];
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        if (i < chunks.length - 1) {
          console.log(
            `Esperando ${delayBetweenChunks / 1000} segundos para siguiente lote...`,
          );
