...
export class CollectionController {
  ...
  @Get('/export')
  async exportInvoicesExcel(@Res({ passthrough: true }) res: FastifyReply) {
    const buffer = await this.collectionService.exportExcelCollection();
    res.header(
      'Content-Type',
      'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
    );
    res.header('Content-Disposition', 'attachment; filename=Cobranza.xlsx');
    return buffer;
  }
