...
export class DashboardService {
  ...
  async generateInventoryAndInvoicesExcelV2(filter: DashboardExcel): Promise<Buffer> {
    ...
    inventarioMovsAntes.forEach(mov => {
      ...
      inventarioInicialMap.set(mov.productId, current + (Number(mov.quantity) * sign));
    });
    ...
    productos.forEach(p => {
      ...
      dias.forEach(dia => {
        const fechaKey = format(dia, 'yyyy-MM-dd');
        const cantidadDespachada: number = despachosPorDiaYProducto[fechaKey][p.id] || 0;
  ...
  async getClientsDemandReport(filter: DashboardExcel) {
    ...
    const bucketClientMap: Record<string, Map<string, {
      clientName: string;
      clientBlock: string;
      invoicesCount: number;
      invoices: Array<{
    ...
    const topClients = Object.values(buckets).flat().sort((a, b) => b.invoicesCount - a.invoicesCount)
