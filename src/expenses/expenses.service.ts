...
export class ExpensesService {
  ...
  private isDivisaPayment(payment: any) {
    ...
    const isCashUsd =
      methodName.includes('efectivo $') || methodName.includes('efectivo usd');
    const isGastoDivisa =
      accountName.includes('gasto divisa') || bankName.includes('gasto divisa');
  ...
  async getInvoicesWithMetrics(expenseFilter: ExpensesDTO) {
    try {
      ...
      const productSales: Record<
        number,
        { name: string; presentation: string; quantity: number }
      > = {};
      let totalQuantity = 0;
      ...
      const result = invoices.map((invoice) => {
        ...
        for (const item of invoice.invoiceItems) {
          ...
          if (isUSD) {
            earn +=
              (Number(item.unitPriceUSD) - purchasePriceUSD) *
              Number(item.quantity);
          } else {
            earn +=
              (Number(item.unitPrice) - purchasePrice) * Number(item.quantity);
          }
        }
        ...
        if (
          invoice.dispatchDate >= startOfDay &&
          invoice.dispatchDate <= endOfDay
        )
          totalEarnDay += earn;
        if (
          invoice.dispatchDate >= startOfMonth &&
          invoice.dispatchDate <= endDate
        ) {
          totalEarnMonth += earn;
        ...
        for (const item of invoice.invoiceItems) {
          ...
          if (!productSales[pid]) {
            productSales[pid] = {
              ...
              presentation: item.product.presentation || 'Desconocido',
              quantity: 0,
            };
          }
      ...
      const productPercentages = Object.entries(productSales)
        .map(([id, data]) => ({
          ...
          percentage:
            totalQuantity > 0
              ? ((data.quantity / totalQuantity) * 100).toFixed(2)
              : '0.00',
        }))
