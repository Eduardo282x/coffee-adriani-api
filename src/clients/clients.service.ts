...
export class ClientsService {
  ...
  async formatNumberClients() {
    ...
    const parseClients = clients.map((cli) => {
      const parseNumber =
        cli.phone.slice(0, 1) === '0' ? cli.phone : `0${cli.phone}`;
      return {
        ...cli,
        phone: parseNumber,
      };
  ...
  async deleteClients(id: number): Promise<DTOBaseResponse> {
    try {
      ...
      baseResponse.message =
        'Cliente y datos relacionados eliminados exitosamente.';
      return baseResponse;
    } catch (err) {
      await this.prismaService.errorMessages.create({
        data: { message: (err as any).message, from: 'ClientService' },
  ...
  async reportClients(
    client: DTOReportClients,
  ): Promise<Buffer | DTOBaseResponse> {
    try {
      ...
      invoiceTotals.forEach((item) => {
        const previous = totalMap.get(item.clientId) || {
          totalAmount: 0,
          remaining: 0,
        };
      ...
      const lastInvoiceMap = new Map<
        number,
        { controlNumber: string; dispatchDate: Date | null }
      >();
      ...
      let clientsReports = clients.map((c) => {
        ...
        return {
          ...
          totalPaid,
          debt,
          lastInvoiceControlNumber: lastInvoice
            ? lastInvoice.controlNumber
            : '',
          lastInvoiceDispatchDate: lastInvoice
            ? lastInvoice.dispatchDate
            : null,
        };
      });
      ...
      const filePDF = await new Promise<Buffer>((resolve, reject) => {
        ...
        doc
          .font('Helvetica-Bold')
          .fontSize(12)
          .text(`Lista de clientes - Producto: ${client.type}`, 10, 20, {
            align: 'left',
          });
        let startY = doc.y + 10;
        drawHeaders(startY);
