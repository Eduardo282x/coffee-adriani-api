...
import {
  AssociatePaymentDTO,
  CreatePaymentForEntryDTO,
  DisassociatePaymentDTO,
} from './entry-payments.dto';
...
export class EntryPaymentsService {
  ...
  async associatePaymentToEntry(data: AssociatePaymentDTO) {
    try {
      ...
      const existingAssociation =
        await this.prismaService.inventoryEntryPayment.findUnique({
          where: {
      ...
      const newTotalPaid = updatedEntry.payments.reduce(
        (sum, p) => sum + Number(p.amount),
        0,
      );
      const newStatus =
        newTotalPaid >= Number(updatedEntry.totalAmount)
          ? 'PAGADO'
          : 'PENDIENTE';
  ...
  async disassociatePaymentFromEntry(data: DisassociatePaymentDTO) {
    try {
      ...
      const newStatus =
        totalPaid >= Number(entry.totalAmount)
          ? 'PAGADO'
          : totalPaid > 0
            ? 'PENDIENTE'
            : 'CREADA';
  ...
  async getPaymentsByEntry(entryId: number) {
    try {
      ...
      const entryPayments =
        await this.prismaService.inventoryEntryPayment.findMany({
      ...
      const payments = entryPayments.map((ep) => {
        const amountUSD = Number(ep.amount);
    ...
    try {
      ...
      const updatedPayment = await this.prismaService.payment.update({
        ...
        data: {
          ...
          accountId:
            data.accountId !== undefined ? data.accountId : payment.accountId,
          reference:
      ...
      const findInventoryEntryPayment =
        await this.prismaService.inventoryEntryPayment.findFirst({
          where: {
            paymentId: paymentId,
          },
        });
