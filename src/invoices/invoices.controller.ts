...
export class InvoicesController {
  ...
  @ApiQuery({
    ...
    type: String,
    required: false,
    description: 'ID del bloque (opcional)',
  })
  @ApiQuery({
    name: 'status',
    type: String,
  ...
  @Put('/lost/:id')
  async markLost(@Param('id', ParseIntPipe) id: number) {
    return await this.invoicesService.markLost(id);
  }

  @Put('/:id')
  async updateInvoice(
    @Param('id', ParseIntPipe) id: number,
    @Body() updatedInvoice: DTOInvoice,
  ) {
